...
static void update_sweep_result(Hit *result, Body *body, usize other_id, vec3 velocity) {
...
static void update_sweep_result_static(Hit *result, Body *body, usize other_id, vec3 velocity) {
...
usize physics_body_create(vec3 position, vec3 size, vec3 velocity, u8 collision_layer, u8 collision_mask, bool is_kinematic, On_Hit on_hit, On_Hit_Static on_hit_static, usize entity_id, int index_trigger, int a,
	int l){
...
usize physics_static_body_create(vec3 position, vec3 size, u8 collision_layer) {
...
usize physics_trigger_create(vec3 position, vec3 size, u8 collision_layer, u8 collision_mask, On_Hit on_hit) {
	
	u32 id = physics_body_create(position, size, (vec3){0, 0, 0}, collision_layer, collision_mask, true, on_hit, NULL, (usize)-1,1,0,0);
...
Static_Body *ray_cast_line(vec3 position, vec3 D, u8 dist){
	...
	for ( u32 n = 0; n < state.static_body_list->len; ++n ) {
