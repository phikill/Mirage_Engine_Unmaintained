...
void on_npc_body_hit_static(Body *self, Static_Body *other, Hit hit) {
...
for ( int n = 0; n < index_text_ui; n++ ){
   for(y=0;y<15;y++){
      for(x=0;x<15; x++){
            ...
            float x1= ( x  * global.M.cos[ 155 - (30 * (n) ) ]) - ( (y - 52) * global.M.sin[ 155 - (30 * (n) )  ]);
            float y1 =( x  * global.M.sin[ 155 - (30 * (n) ) ]) + ( (y - 52) * global.M.cos[ 155 - (30 * (n) )  ]);
  ...
  for(x=20;x<103; x++){
    for (y=90;y<117; y++){
            float x1= (x - 60) * global.M.cos[P->a] - ((y - 80) - 60) * global.M.sin[P->a];
            float y1 =(x - 60) * global.M.sin[P->a] + ((y - 80) - 60) * global.M.cos[P->a];
            ...
            int st = 23;
            int pixel= (int)(textures[st].h -((int)yrotated%textures[st].h) -1)  *3* textures[st].w + ((int)xrotated%textures[st].w)*3 ;
...
int  drawWall(int x1,int x2, int b1,int b2, int t1,int t2,int s,int w, int frontBack, TextureMaps* textures, Body* P,math M)
{
 ...
 int dx   = (float)( x2-x1 ); if( dx==0){ dx=1;}    //x distance
 float ht_step =  (( (float)textures[wt].w ) )  / ((float) x2- ((float)x1 + (float)(x2-x1) * .5f));

 int xs=x1;//hold initial x1 starting position
 if(x1<0){  ht -= ht_step * (float)( x1 ); x1 = 0;} //clip left
 ...
 for(x=x1;x<x2;x++)
 {
  ...
  //Clip Y
  if(y1< 0){ vt -= vt_step * (float)y1; y1= 0;} //clip y
  if(y2< 0){ y2= 0;} //clip y
 ...
 if (frontBack == 1){
    ...
    float moveUpDown = (float)(P->aabb.position[2] - w0) / (float)( y0 );  if(moveUpDown == 0) moveUpDown = 0.001;
    ...
    for (y=ys; y < ye; y++){
            ...
            float z = y + lookUpAnDown;   if (z  ==0 ) { z  = 0.0001; }
...
void draw_characters( TextureMaps* textures, Body*  P,math M ){
    ...
    if ( entity_player != NULL ){
            ...
            Static_Body *hit_ = ray_cast_line(
                              (vec3){
                                    P->aabb.position[0],
                                    P->aabb.position[1],
                                    P->aabb.position[2]}
            ...
            D[1] *= 12;
				...
				if ( hit != NULL ) {
			   	if ( entity_get(hit->entity_id) != NULL ) {
                     draw3D_character_at_location(
                     entity_get(hit->entity_id),
                     textures, P, M, entity_get(hit->entity_id)->texture_id);
                  }
             }
      }
}
