...
for(s=0;s<numSect;s++){
   ...
   if( y2 - y1  < 0  ){ int swp=y1; y1=y2; y2=swp;}
...
on_time_out_tree_delay delay_create_character(u32 id, u32 value, f32 current_time){

   for ( int index_char = 0; index_char < (int)value; index_char++ ){
      if (next_character[index_char].x + next_character[index_char].y > 0 ){
         u32 entity_id = init_characters((vec3){ next_character[index_char].x, next_character[index_char].y, CHARACTER_DEFAULT_SIZE},
         NULL, NULL, next_character[index_char].i,0, 0, 0, next_character[index_char].a, next_character[index_char].l, 0);
...
for( u32 index_entity = (u32)entity_array_list_get()->len;  index_entity > 0; --index_entity ){
...
for( u32 physics_id = (u32)physics_body_list_array_get()->len;  physics_id > 0; --physics_id){
...
for( u32 physics_id = (u32)physics_static_body_list_array_get()->len;  physics_id > 0; --physics_id){
...
physics_reset();
	...
	*P_viewr  =  entity_create(NULL, CAMERA,  (vec3){ 40, 50, 110 }, (vec3){
	 CHARACTER_DEFAULT_SIZE,
	 CHARACTER_DEFAULT_SIZE,
	  CHARACTER_DEFAULT_SIZE },
	  (vec2){416,416},(vec3){0,0,0 },  COLLISION_LAYER_PLAYER, player_mask,  false, new_char_player_id, NULL, NULL,  WALK, 0, 100, On_Entity_Delete_Player, NULL, true, 0, 0, 0, 0,0,0);
...
fclose(fp);
create_time_tree_timer(delay_create_character, (u32)size_char, (u32)size_char, 2.2f  );
