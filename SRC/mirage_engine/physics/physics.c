#include <linmath\linmath.h>
#include "../global.h"
#include "../array_list/array_list.h"
#include "../util.h"
#include "physics.h"
#include "../types.h"
#include<math.h>

#ifdef __WATCOMC__
	float fmaxf(float x, float y) 
	{
    	return (x > y) ? x : y;
	}
	float fminf(float x, float y) 
	{
    	return (x < y) ? x : y;
	}
	float fabsf(float x) 
	{
    	return (x < 0) ? -x : x;
	}
#endif

typedef struct physics_state_internal 
{
	f32 gravity;
	f32 terminal_velocity;
	Array_List *body_list;
	Array_List *static_body_list;
} Physics_State_Internal;

static Physics_State_Internal state;

static u32 iterations = 60;
static f32 tick_rate;

void aabb_min_max(vec3 min, 
				  vec3 max, 
				  AABB aabb) 
{
	vec3_sub(min, aabb.position, aabb.half_size);
	vec3_add(max, aabb.position, aabb.half_size);
}

AABB aabb_minkowski_difference(AABB a, AABB b) 
{
	AABB result;
	vec3_sub(result.position, a.position, b.position);
	vec3_add(result.half_size, a.half_size, b.half_size);

	return result;
}

Hit ray_intersect_aabb(vec3 pos, 
					   vec3 magnitude, 
					   AABB aabb) 
{
	Hit hit = {0};
	vec3 min, max;
	f32 last_entry;
	f32 first_exit;
	u8 i;


	aabb_min_max(min, max, aabb);

	last_entry = -INFINITY;
	first_exit = INFINITY;

	for(i = 0; i < 2; ++i) 
	{
		if(magnitude[i] != 0) 
		{
			f32 t1 = (min[i] - pos[i]) / magnitude[i];
			f32 t2 = (max[i] - pos[i]) / magnitude[i];

			last_entry = fmaxf(last_entry, fminf(t1, t2));
			first_exit = fminf(first_exit, fmaxf(t1, t2));
		} 
		else if (pos[i] <= min[i] || pos[i] >= max[i]) 
		{
			return hit;
		}
	}

	if(first_exit > last_entry && first_exit > 0 && last_entry < 1) 
	{
		f32 dx, dy, px, py;

		hit.position[0] = pos[0] + magnitude[0] * last_entry;
		hit.position[1] = pos[1] + magnitude[1] * last_entry;

		hit.is_hit = true;
		hit.time = last_entry;

		dx = hit.position[0] - aabb.position[0];
		dy = hit.position[1] - aabb.position[1];
		px = aabb.half_size[0] - fabsf(dx);
		py = aabb.half_size[1] - fabsf(dy);

		if(px < py) 
		{
			hit.normal[0] = (dx > 0) - (dx < 0);
		} 
		else 
		{
			hit.normal[1] = (dy > 0) - (dy < 0);
		}
	}
	return hit;
}

bool physics_aabb_intersect_aabb(AABB a, AABB b) 
{
	vec3 min, max;
	aabb_min_max(min, max, aabb_minkowski_difference(a, b));

	return (min[0] <= 0 && max[0] >= 0 && min[1] <= 0 && max[1] >= 0);
}

void aabb_penetration_vector(vec3 r, AABB aabb) 
{
	vec3 min, max;
	f32 min_dist;


	aabb_min_max(min, max, aabb);

	min_dist = fabsf(min[0]);
	r[0] = min[0];
	r[1] = 0;

	if (fabsf(max[0]) < min_dist) 
	{
		min_dist = fabsf(max[0]);
		r[0] = max[0];
	}

	if(fabsf(min[1]) < min_dist) 
	{
		min_dist = fabsf(min[1]);
		r[0] = 0;
		r[1] = min[1];
	}

	if(fabsf(max[1]) < min_dist) 
	{
		r[0] = 0;
		r[1] = max[1];
	}
}

bool physics_point_intersect_aabb(vec3 point, AABB aabb) 
{
	vec3 min, max;
	aabb_min_max(min, max, aabb);
	return  point[0] >= min[0] &&
		point[0] <= max[0] &&
		point[1] >= min[1] &&
		point[1] <= max[1];
}

void physics_init(void) 
{
	state.body_list = array_list_create(sizeof(Body), 0);
	state.static_body_list = array_list_create(sizeof(Static_Body), 0);

	state.gravity = -79;
	state.terminal_velocity = -7000;

	tick_rate = 1.f / iterations;
}

static void update_sweep_result(Hit *result, 
								Body *body, 
								usize other_id, 
								vec3 velocity) 
{
	Body *other;
	AABB sum_aabb;
	Hit hit;

	other = physics_body_get(other_id);

	if((body->collision_mask & other->collision_layer) == 0) 
	{
		result->trigger = true;
	}

	sum_aabb = other->aabb;
	vec3_add(sum_aabb.half_size, sum_aabb.half_size, body->aabb.half_size);

	hit = ray_intersect_aabb(body->aabb.position, velocity, sum_aabb);
	if(hit.is_hit) 
	{
		if(body->on_hit && (body->collision_mask & other->collision_layer) == 0) 
		{
			body->on_hit(body, other, hit);
		}

		if(hit.time < result->time) 
		{
			*result = hit;
		} 
		else if(hit.time == result->time) 
		{
			// Solve highest velocity axis first.
			if (fabsf(velocity[0]) > fabsf(velocity[1]) && hit.normal[0] != 0) 
			{
				*result = hit;
			} 
			else if (fabsf(velocity[1]) > fabsf(velocity[0]) && hit.normal[1] != 0) 
			{
				*result = hit;
			}
		}
		result->other_id = other_id;
	}
}

static void update_sweep_result_static(Hit *result, 
									   Body *body, 
									   usize other_id, 
									   vec3 velocity) 
{
	Static_Body *static_body;
	AABB sum_aabb;
	Hit hit;

	static_body = physics_static_body_get(other_id);

	if((body->collision_mask & static_body->collision_layer) == 0) 
	{
		return;
	}

	sum_aabb = static_body->aabb;
	vec3_add(sum_aabb.half_size, sum_aabb.half_size, body->aabb.half_size);

	hit = ray_intersect_aabb(body->aabb.position, velocity, sum_aabb);
	if(hit.is_hit) 
	{
		if(hit.time < result->time) 
		{
			*result = hit;
		} 
		else if(hit.time == result->time) 
		{
			// Solve highest velocity axis first.
			if(fabsf(velocity[0]) > fabsf(velocity[1]) && hit.normal[0] != 0) 
			{
				*result = hit;
			} 
			else if(fabsf(velocity[1]) > fabsf(velocity[0]) && hit.normal[1] != 0) 
			{
				*result = hit;
			}
		}
		result->other_id = other_id;
	}
}

static Hit sweep_static_bodies(Body *body, vec3 velocity)
{
	u32 i;
	Hit result;

	//Hit result = {.time = 0xBEEF};

	result.time = 0xBEEF;

	for(i = 0; i < state.static_body_list->len; ++i) 
	{
		update_sweep_result_static(&result, body, i, velocity);
	}

	return result;
}

static Hit sweep_bodies(Body *body, vec3 velocity) 
{
	u32 i;
	Hit result;

	//Hit result = {.time = 0xBEEF};

	result.time = 0xBEEF;

	for(i = 0; i < state.body_list->len; ++i) 
	{
		Body *other = physics_body_get(i);

		if (body == other) 
		{
			continue;
		}
		update_sweep_result(&result, body, i, velocity);
	}
	return result;
}

static void sweep_response(Body *body, vec3 velocity) 
{
	Hit hit = sweep_static_bodies(body, velocity);
	Hit hit_moving = sweep_bodies(body, velocity);

	if(hit_moving.is_hit) 
	{
		if (body->on_hit != NULL) 
		{
			body->on_hit(body, physics_body_get(hit_moving.other_id), hit_moving);
		}
	} 

	if(hit.is_hit) 
	{
		body->aabb.position[0] = hit.position[0];
		body->aabb.position[1] = hit.position[1];

		if(hit.normal[0] != 0) 
		{
			body->aabb.position[1] += velocity[1];
			body->velocity[0] = 0;
		} 
		else if (hit.normal[1] != 0) 
		{
			body->aabb.position[0] += velocity[0];
			body->velocity[1] = 0;
		}

		if(body->on_hit_static != NULL) 
		{
			body->on_hit_static(body, physics_static_body_get(hit.other_id), hit);
		}
	} 
	else 
	{
		vec3_add(body->aabb.position, body->aabb.position, velocity);
	}
}

static void stationary_response(Body *body) 
{
	u32 i;
	//usize i;

	for(i = 0; i < state.static_body_list->len; ++i) 
	{
		Static_Body *static_body;
		AABB aabb;
		vec3 min, max;

		static_body = physics_static_body_get(i);

		if((body->collision_mask & static_body->collision_layer) == 0) 
		{
			continue;
		}

		aabb = aabb_minkowski_difference(static_body->aabb, body->aabb);
		aabb_min_max(min, max, aabb);

		if(min[0] <= 0 && max[0] >= 0 && min[1] <= 0 && max[1] >= 0) 
		{
			vec3 penetration_vector;
			aabb_penetration_vector(penetration_vector, aabb);

			vec3_add(body->aabb.position, body->aabb.position, penetration_vector);
		}
	}

	// Check for on-hit events.
	for(i = 0; i < state.body_list->len; ++i) 
	{
		Body *other;
		AABB aabb;
		vec3 min, max;

		other = physics_body_get(i);

		if(!body->on_hit) 
		{
			continue;
		}

		if((body->collision_mask & other->collision_layer) == 0) 
		{
			continue;
		}

		aabb = aabb_minkowski_difference(other->aabb, body->aabb);
		aabb_min_max(min, max, aabb);

		if(min[0] <= 0 && max[0] >= 0 && min[1] <= 0 && max[1] >= 0) 
		{
			Hit hit;
			hit.is_hit = 1;
			body->on_hit(body, other, hit);

			//body->on_hit(body, other, (Hit){.is_hit = true, .other_id = i});
		}
	}
}

void physics_update(void) 
{
	u32 i, j;
	Body *body;
	vec3 scaled_velocity;

	for(i = 0; i < state.body_list->len; ++i) 
	{
		body = array_list_get(state.body_list, i);

		if(!body->is_active) 
		{
			continue;
		}

		if(!body->is_kinematic) 
		{
			//body->velocity[1] += state.gravity;
			if (state.terminal_velocity > body->velocity[1]) 
			{
				body->velocity[1] = state.terminal_velocity;
			}
		}

		body->velocity[0] += body->acceleration[0];
		body->velocity[1] += body->acceleration[1];

		vec3_scale(scaled_velocity, body->velocity, global.time.delta * tick_rate);

		for(j = 0; j < iterations; ++j) 
		{
			sweep_response(body, scaled_velocity);
			stationary_response(body);
		}
	}
}

usize physics_body_create(vec3 position, 
						  vec3 size, 
						  vec3 velocity, 
						  u8 collision_layer, 
						  u8 collision_mask, 
						  bool is_kinematic, 
						  On_Hit on_hit, 
						  On_Hit_Static on_hit_static, 
						  usize entity_id, 
						  int index_trigger, 
						  int a,
						  int l)
{
	usize i;
    usize id = state.body_list->len;
    Body *body;
    Body temp_body;

	// Find inactive Body.
	for(i = 0; i < state.body_list->len; ++i) 
	{
		Body *body = array_list_get(state.body_list, i);
		if(!body->is_active) 
		{
			id = i;
			break;
		}
	}

	if(id == state.body_list->len) 
	{
		Body temp_body = {0};

		//if(array_list_append(state.body_list, &(Body){0}) == (usize)-1) 
		if(array_list_append(state.body_list, &temp_body) == (usize) -1) 
		{
			ERROR_EXIT("Could not append body to list\n");
		}
	}

	body = physics_body_get(id);

	temp_body.aabb.position[0] = position[0];
	temp_body.aabb.position[1] = position[1];
	temp_body.aabb.position[2] = position[2];

	temp_body.aabb.half_size[0] = size[0] * 0.5;
	temp_body.aabb.half_size[1] = size[1] * 0.5;
	temp_body.aabb.half_size[2] = size[2] * 0.5;

	temp_body.velocity[0] = velocity[0];
	temp_body.velocity[1] = velocity[1];

	temp_body.collision_layer = collision_layer;
	temp_body.collision_mask = collision_mask;
	temp_body.on_hit = on_hit;
	temp_body.on_hit_static = on_hit_static;
	temp_body.is_kinematic = is_kinematic;
	temp_body.is_active = 1; // true é equivalente a 1
	temp_body.entity_id = entity_id;
	temp_body.index_trigger = index_trigger;
	temp_body.a = a;
	temp_body.l = l;

	*body = temp_body;

	/*
	*body = (Body)
	{
		.aabb = 
		{
			.position = 
			{ 
				position[0], 
				position[1], 
				position[2]  
			},
			
			.half_size = 
			{ 
				size[0] * 0.5, 
				size[1] * 0.5, 
				size[2] * 0.5 
			},
		},
		
		.velocity = 
		{ 
			velocity[0], 
			velocity[1] 
		},
		
		.collision_layer = collision_layer,
		.collision_mask = collision_mask,
		.on_hit = on_hit,
		.on_hit_static = on_hit_static,
		.is_kinematic = is_kinematic,
		.is_active = true,
        .entity_id = entity_id,
		.index_trigger   = index_trigger,
		.a   = a,
		.l   = l,
	};*/
	return id;
}

Body *physics_body_get(usize index) 
{
	return array_list_get(state.body_list, index);
}

usize physics_static_body_create(vec3 position, 
								 vec3 size, 
								 u8 collision_layer) 
{
	Static_Body static_body;

	static_body.aabb.position[0] = position[0];
	static_body.aabb.position[1] = position[1];
	static_body.aabb.position[2] = position[2];

	static_body.aabb.half_size[0] = size[0] * 0.5;
	static_body.aabb.half_size[1] = size[1] * 0.5;
	static_body.aabb.half_size[2] = size[2] * 0.5;

	static_body.collision_layer = collision_layer;

	/*
	Static_Body static_body =
	{
		.aabb = 
		{
			.position = 
			{ 
				position[0], 
				position[1], 
				position[2]  
			},
			.half_size = 
			{ 
				size[0] * 0.5, 
				size[1] * 0.5, 
				size[2] * 0.5  
			},
		},
		.collision_layer = collision_layer,
	};*/

	if (array_list_append(state.static_body_list, &static_body) == (usize)-1)
	{
		ERROR_EXIT("Could not append static body to list\n");
	}

	return state.static_body_list->len - 1;
}

usize physics_trigger_create(vec3 position, 
							 vec3 size, 
							 u8 collision_layer, 
							 u8 collision_mask, 
							 On_Hit on_hit) 
{
	u32 id;
	vec3 vector_mango = {0, 0, 0};
	usize invalid_usize = (usize)-1;

	id = physics_body_create(position, 
                         	size, 
                         	vector_mango, 
                         	collision_layer, 
                         	collision_mask, 
                         	true, 
                         	on_hit, 
                         	NULL, 
                         	invalid_usize, 
                         	1, 
                         	0, 
                         	0);
	/*
	id = physics_body_create(position, 
							 size, 
								 (vec3){0, 0, 0}, 
								 collision_layer, 
								 collision_mask, 
								 true, 
								 on_hit, 
								 NULL, 
								 (usize)-1,1,0,0);*/
	physics_body_get(id)->entity_id = -1000;
    return id;

}


void static_physics_body_destroy_from_memory(usize body_id) 
{
	array_list_remove(state.static_body_list, body_id);
}

void physics_body_destroy_from_memory(usize body_id) 
{
	array_list_remove( state.body_list, body_id);
}

void physics_static_body_destroy_from_memory(usize body_id) 
{
	array_list_remove( state.static_body_list, body_id);
}

Array_List *physics_static_body_list_array_get()
{
	return state.static_body_list;
}

Array_List *physics_body_list_array_get()
{
	return state.body_list;
}

Static_Body *physics_static_body_get(usize index) 
{
	return array_list_get(state.static_body_list, index);
}

usize physics_static_body_count() 
{
    return state.static_body_list->len;
}

void physics_reset() 
{	
  	state.body_list->len 		= 0;
  	state.static_body_list->len = 0;
}

void physics_body_destroy(usize body_id) 
{
    Body *body = physics_body_get(body_id);
	body->is_active = false;
}

Static_Body *ray_cast_line(vec3 position, vec3 D, u8 dist)
{
	u32 n;
	f32 X = position[0];
	f32 Y = position[1];
	f32 Z =	position[2];

	for(n = 0; n < state.static_body_list->len; ++n ) 
	{
		Static_Body * s_other = physics_static_body_get(n);
		if(s_other)
		{
			Hit result;
			AABB sum_aabb;
			Body body = {0};
			vec3 velocity;
			Hit hit;

			result.time = 0XBEEF;

			//Hit result = (Hit){.time = 0xBEEF};
			sum_aabb = s_other->aabb;

			body.aabb.position[0] = X;
			body.aabb.position[1] = Y;
			body.aabb.position[2] = Z;

			body.aabb.half_size[0] = 3;
			body.aabb.half_size[1] = 3;
			body.aabb.half_size[2] = 1;

			vec3_add(sum_aabb.half_size, 
					 sum_aabb.half_size, 
					 body.aabb.half_size);
			//vec3 velocity = { D[0], D[1], 0 };
			velocity[0] = D[0];
			velocity[1] = D[1];
			velocity[2] = D[2];

			hit = ray_intersect_aabb(body.aabb.position, velocity, sum_aabb);
			if(hit.is_hit)
			{
				return s_other;
			}
		}
	}
	return NULL;
}

