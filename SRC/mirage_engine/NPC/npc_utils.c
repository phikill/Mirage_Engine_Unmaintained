					...
					NPC_Alignment aligment)
{
	...
	usize char_player_id;
	u8 animation_frames[5] = {1, 2, 3, 4, 5};
	...
	player_mask = COLLISION_LAYER_ENEMY
    ...
    char_player_id = animation_definition_create(&sprite_sheet_player, ANIMATION_DURATION, 0, animation_frames, 5);
    //char_player_id = animation_definition_create(&sprite_sheet_player, ANIMATION_DURATION, 0, (u8[]){1, 2, 3, 4,5}, 5);
	...
	position_vec[0] = position[0];
	position_vec[1] = position[1];
	position_vec[2] = position[2];

	size_vec[0] = CHARACTER_DEFAULT_SIZE;
	size_vec[1] = CHARACTER_DEFAULT_SIZE;
	size_vec[2] = CHARACTER_DEFAULT_SIZE;
	...
	entity_npc = entity_create(mind, 
                            	...
                            	false, 
                            	new_char_player_id, 
                            	on_npc_player_body_hit, 
                            	on_npc_player_body_hit_static,  
                            	WALK, 
                            	0, 
                            	CHARACTER_DEFAULT_LIFE, 
                            	On_Entity_Delete_NPC, 
                            	value_npc_change, 
                            	false, 
                            	index, 
                            	0, 
	...
	/*
	entity_npc = entity_create(mind, RIGHT, 
									(vec3)
									{ 
										position[0], 
										position[1], 
