...
usize array_list_append(Array_List *list, void *item) 
{
	...
	if(list->len == list->capacity) 
	{
		...
		items = realloc(list->items, list->item_size * list->capacity);
...
void *array_list_get(Array_List *list, usize index) 
{
	if(index >= list->len)
	{
		 return NULL;  // ERROR_RETURN(NULL, "Index out of bounds\n");
	}
...
u8 array_list_remove(Array_List *list, usize index) 
{
	...
	item_ptr = (u8*)list->items + index * list->item_size;
	end_ptr = (u8*)list->items + list->len * list->item_size;
